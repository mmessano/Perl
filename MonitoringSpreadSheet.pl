...
$obook = "C:\\Dexma\\Logs\\MonitoredServers.xls";
...
# header row
@header =  (
	[ "Server", "Env", "CPU", "Memory", "Disk", "DSN", "DCOM", "SchedTasks", "Queues", "PerfMon", "IISSites", "DRM", "Active" ]
	);
...
$SQLSEL = "select s.server_name AS Server, \
		  ...
		  AND s.server_name not like '%Unknown%'  \
...
print "Begin updating spreadsheet...\n";
while( $DSN->FetchRow() ) {
 	%Data = $DSN->DataHash();
